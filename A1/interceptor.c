...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// should be ax instead of orig_ax
	mytable callMessage = table[reg.ax];
	if (callMessage.intercepted == 1)	{
		// check if pid is monitored
		if(check_pid_monitored(reg.ax, current->pid))
		{
			 log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		}
	    // might not be monitored -> for something else, still want to log it
		else if (callMessage.monitored == 2) {
			log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		}
	}
	return callMessage.f(reg);
}
...
376  *   Use the helper functions provided above for dealing with list operations.
 *
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// cmd checking
	if ( cmd != REQUEST_SYSCALL_RELEASE && cmd != REQUEST_SYSCALL_INTERCEPT && cmd != REQUEST_START_MONITORING && cmd != REQUEST_STOP_MONITORING)
		return -EINVAL;
	// syscall number checking
	if (syscall < 0 || syscall > NR_syscalls || syscall == MY_CUSTOM_SYSCALL)
		return -EINVAL;
	
	// pid checking
	// must not be a negative number and must be an existing pid
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING){
		if (pid < 0 || (pid_task(find_vpid(pid),PIDTYPE_PID) == NULL && (pid !=0)))//could have typed in 0 -? which is just a flag? 
			return -EINVAL;
	}
	...
	if (current_uid() != 0) {
		...
		// for the last two cases
   		if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) {
			if ( pid == 0 || check_pids_same_owner(current->pid, pid) != 0  ) //is not root, but want to clear all -> they cant
				return -EPERM;
		}				
	}

	// cannot release a intecept that is not intercepted
	if ( cmd == REQUEST_SYSCALL_RELEASE) {
		if (table[syscall].intercepted == 0) 
			return -EINVAL;
	}
	//  cannot del a pid that is not being monitored ot intercepted
	if (cmd  == REQUEST_STOP_MONITORING) {
		if ((check_pid_monitored(syscall, pid) == 0 && pid != 0 ) || table[syscall].intercepted == 0)
			return -EINVAL;
	}	


	// cannot intersept a syscall that is already intersepted
	if ( cmd == REQUEST_SYSCALL_INTERCEPT) {
		if (table[syscall].intercepted == 1)
			return -EBUSY;
	}

	// cannot moniter a pid that is being monitered
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT ) {
		...
		//system call has now been intercepted
		table[syscall].intercepted = 1;
		spin_unlock( &my_table_lock);

		spin_lock( &sys_call_table_lock );
		set_addr_rw((unsigned long)sys_call_table); 
		//replace the system call with our intercepter
		...
		spin_lock( &sys_call_table_lock );
		set_addr_rw((unsigned long)sys_call_table);
		//restore the original systemcall
		sys_call_table[syscall] = table[syscall].f;
		set_addr_ro((unsigned long)sys_call_table);
		spin_unlock( &sys_call_table_lock);

		spin_lock( &my_table_lock);
		//restore the structs in our table to default values
		...
		if (pid == 0)
			...
			// set to monitored, add the pid
			int check = add_pid_sysc(pid, syscall);
			if( check == 0){  
		...
		if ( pid == 0) {
			destroy_list(syscall);
			//set monitor to 0 already done in function
		} else {
			int check = del_pid_sysc(pid, syscall);
			if (check != 0) {
				spin_unlock( &my_table_lock);
				return check;
			}
...
static int init_function(void) {
	int index;
	// init the locks, or the program will get stuck
	spin_lock_init( &sys_call_table_lock);
	spin_lock_init( &my_table_lock);

	// lock the syscall table
	spin_lock( &sys_call_table_lock);
	// enable rw of syscall table
	set_addr_rw((unsigned long)sys_call_table); //set the sys call table for read write
	// save the orig my_sys_call
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	// change the table item to my syscall
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;
	// do the same funny thing to exit group
	orig_exit_group = sys_call_table[__NR_exit_group];
	sys_call_table[__NR_exit_group] = my_exit_group;
	// disable rw of the syscall table
	set_addr_ro((unsigned long)sys_call_table);
	//	unlock sys call table
	spin_unlock( &sys_call_table_lock);

	// bookeeping, setup the mytable struct for each syscall 
	// lock my table
	spin_lock( &my_table_lock);	
...
{        
	...
	// debug messege
	spin_lock( &sys_call_table_lock);
	// set the sys call table for read write
	set_addr_rw((unsigned long)sys_call_table);
	// reset the custom one
	sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall;
	// reset the exit group
	sys_call_table[__NR_exit_group] = orig_exit_group;

	//disable modifying the table
	set_addr_ro((unsigned long)sys_call_table);
	// unlock my table
	spin_unlock( &sys_call_table_lock);

	//lock my table
	for (index = 1; index < NR_syscalls; index++) {
		// destroy the lists of process
		destroy_list(index);
	}
}
