...
int main(int argc, char **argv) {
   ...
   // printf("%s is the \n", root_test);
    ...
    if (root_test == NULL) {
        ...
        r = 2;
        //how do we restore the first entry? Dont think we can
        ...
        if (r == -1) {
		    return EEXIST; //this value is 17 may want to do an exit instead.
	    }
	    else if (r == ENOENT) {
		    return ENOENT;
        }
        //r is the inode we are searching in
    }
    ...
    for (int i = 0; i < 13; i++) {
        if (i == 12) {
	    	for(int y = 0; y < EXT2_BLOCK_SIZE/sizeof(unsigned int) ;y++) {
                ...
                int result = check_with_gaps(indirect_table[y],r, tempstr3);
			    if (result) {
                    //printf("we got it\n");
                    //upate the inode
                    //inodes[r-1].
            ...
            int result = check_with_gaps(inodes[r-1].i_block[i],r, tempstr3);
            if (result) {
                // for testing printf("we got it\n");
                break;
                //sucessful- must also do other things with inode and block descriptor
            }
            else if (result == -1) { //found it but it was a directory
                //printf("its a directory\n");
                return EEXIST;
